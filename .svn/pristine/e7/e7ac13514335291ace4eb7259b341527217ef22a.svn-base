...
class CCompressorPlant :
	public CDieselPlant
{
public:
	CCompressorPlant(void);
	~CCompressorPlant(void);
...
protected:
	...
	vector<float> CalcSpecEigValue(const vector<float>& avfWave_,float fDf_,int nRev_);

	bool JudgeAccChannAlarm(float lfCharValue_,CString strFolderName_, int &AlarmCylinderNo_,float& fDistance_);

	bool JudgeDisChannAlarm(float lfCharValue_,CString strFolderName_, int &AlarmCylinderNo_,float& fDistance_);

	BOOL ConfirmAlarm(float fCharValue_,CString strFolderName_, int &AlarmCylinderNo_,float& fDistance_);
