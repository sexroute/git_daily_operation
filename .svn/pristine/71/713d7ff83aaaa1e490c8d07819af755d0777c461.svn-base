#include "StdAfx.h"
#include ".\dieselplant16pa6stc.h"


extern "C" __declspec(dllexport) CDieselPlant* CreateObjectof16PA6STC()
{
	return new CDieselPlant16PA6STC();
}

CDieselPlant16PA6STC::CDieselPlant16PA6STC(void)
{
	m_fAlarmValue = 0;
	this->m_pWave = NULL;
	this->m_nWaveNum = 0;
...
int CDieselPlant16PA6STC::JudgeVibDieselAlarm(float fCharValue_, CString strFolderName_,int &AlarmCylinderNo_,float& fDistance_)
{
...
BOOL CDieselPlant16PA6STC::ReadFolderNameFromDisk( CString & strFolderPath_,const CString &strFolderName_)
{
...
BOOL CDieselPlant16PA6STC::ReadFileNameFromDisk(const CString& strFolderPath_, vector<CString>& vFileName_,bool bSegWave/*=true*/,E_DIESEL_CALC_TYPE eType_/*=E_CALC_WAVE*/ )
{
	CFileFind lfileFind;
	vFileName_.clear();
...
BOOL CDieselPlant16PA6STC::InitModelFilePath()
{
...
float CDieselPlant16PA6STC::CalcSegWaveEnergy(vector<float>& vfData,E_DIESEL_CALC_TYPE nType_)
{
	int lnDimension = GetStableModelDimension();
	...
	if(m_pWave && m_nWaveNum > 0)
	{
		...
		lnWaveNum = GetNextPowPtNum(m_nWaveNum);
		...
		if(ZoomSpectrumAnalysis(m_pWave,m_nWaveNum,lnSampleFreq,lpFreq,lnLines,lfDf,lfStartFreq) != -1)
		{
			...
			{
				int lnPtNum = this->m_nWaveNum;
				if(lnPtNum > this->m_nTwoCyclePtNum)
				{
					lnPtNum = this->m_nTwoCyclePtNum;
				}

				vfData = this->CalcDataSegEnergyPercentage(m_pWave,lnPtNum,lnDimension);
			}
...
CString CDieselPlant16PA6STC::GetModelFolderPath()
{
	return this->m_strFolderPath;
}

void CDieselPlant16PA6STC::SetModelFolderPath( const CString& strPath_ )
{
	this->m_strFolderPath = strPath_;
}


BOOL CDieselPlant16PA6STC::ChannWaveEnergyAlarm(const CString& astrPath_)
{
	//缸起始索引存放在自定义参数中
	float lfEnergy = 0;
...
float CDieselPlant16PA6STC::CalcRadius( const vector<float>& avfData_,const vector<float>& avfOrgCoord_ )
{
	int  lnDimension = GetStableModelDimension();
...
bool CDieselPlant16PA6STC::SetWaveData( const vector<DATATYPE_WAVE>& vWave_ )
{
	this->m_vWave.clear();
...
bool CDieselPlant16PA6STC::SetWaveData( float *pWave_ )
{
	this->m_pWave = pWave_;
...
bool CDieselPlant16PA6STC::SetWaveSegInfo( const vector<DATATYPE_WAVE>& vWaveSegInfo_ )
{
...
bool CDieselPlant16PA6STC::SetFlameOutCylinder( const vector<int>& vFlameOutCylinder_ )
{
...
bool CDieselPlant16PA6STC::SetChannInfo( const S_ChannInfo& sChannInfo_ )
{
	this->m_stChannInfo = sChannInfo_;
	return true;
}

bool CDieselPlant16PA6STC::SetPlantInfo( CString strGroup_,const S_PlantInfo& sPlantInfo_)
{
...
vector<int> CDieselPlant16PA6STC::GetChannFireSequence( const vector<int>& avnSequence_)
{
...
float CDieselPlant16PA6STC::CalcValveEnergy(const vector<float>& avfData_,
...
int CDieselPlant16PA6STC::JudgeFlameOut( const vector<float>& avfData_,
										const vector<int>& avnSequence_,
										float fValveEnergy_,
										float& afDistance_ )
{
...
int CDieselPlant16PA6STC::JudgeCylinderKnock( const vector<float>& avfData_,
											 const vector<int>& avnSequence_,
											 float fValveEnergy_,
											 float& afDistance_ )
{
...
int CDieselPlant16PA6STC::JudgeWattAbrasion( const vector<float>& avfData_,
											const vector<int>& avnSequence_,
											float fValveEnergy_,
											float& afDistance_ )
{
...
int CDieselPlant16PA6STC::JudgeDynDieselAlarm(float lfCharValue_,CString strFolderName_, int &AlarmCylinderNo_,float& fDistance_)
{
	return 0;
}

int CDieselPlant16PA6STC::JudgeProcDieselAlarm(float lfCharValue_,CString strFolderName_, int &AlarmCylinderNo_,float& fDistance_)
{
	return 0;
}

bool CDieselPlant16PA6STC::SetValveEnergy( float fValveEnergy_ )
{
	this->m_fValveEnergy = fValveEnergy_;
	return true;
}

float CDieselPlant16PA6STC::GenNormalData( const vector<float>& vfSegData_,const vector<int>& vfFireSequence_,vector<float>& vfNormalData_ )
{
...
float CDieselPlant16PA6STC::GenFlameOutData( const vector<float>& vfSegData_,const vector<int>& vfFireSequence_,vector<float>& vfFlameoutData_ )
{
	if(vfSegData_.size() <= 0 || vfSegData_.size() != vfFireSequence_.size())
	{
		return 0;
...
float CDieselPlant16PA6STC::GenCylinderKnockData( const vector<float>& vfSegData_,const vector<int>& vfFireSequence_,vector<float>& vfKnockData_ )
{
	if(vfSegData_.size() <= 0 || vfSegData_.size() != vfFireSequence_.size())
	{
		return 0;
...
float CDieselPlant16PA6STC::GenWattAbrasionData( const vector<float>& vfSegData_,const vector<int>& vfFireSequence_,vector<float>& vfWattAbrasionData_ )
{
	if(vfSegData_.size() <= 0 || vfSegData_.size() != vfFireSequence_.size())
	{
		return 0;
...
float CDieselPlant16PA6STC::CalcNormalDistance( const vector<float>& vfSegData_ )
{
	float lfDistance = 0;
...
float CDieselPlant16PA6STC::CalcFlameOutdistance( const vector<float>& vfSegData_ )
{
	float lfDistance = 0;
...
float CDieselPlant16PA6STC::CalcCylinderKnockDistance( const vector<float>& vfSegData_ )
{
	float lfDistance = 0;
...
float CDieselPlant16PA6STC::CalcWattAbrasionDistance( const vector<float>& vfSegData_ )
{
	float lfDistance = 0;
...
int CDieselPlant16PA6STC::JudgeVibChannAlarm( float afCharValue_,CString strFolderName_, int &AlarmCylinderNo_,float& fDistance_ )
{
	this->ClearData();
...
int CDieselPlant16PA6STC::UpdateNormalData( const vector<float>& avfData_ )
{
...
int CDieselPlant16PA6STC::UpdateFlameOutData( const vector<float>& avfData_ )
{
...
int CDieselPlant16PA6STC::UpdateCynlinderKnockData( const vector<float>& avfData_ )
{
	int lnDataSize = GetInitStableVectorSpaceSize();
...
int CDieselPlant16PA6STC::UpdateWattAbrasionData( const vector<float>& avfData_ )
{
	int lnDataSize = GetInitStableVectorSpaceSize();
...
int CDieselPlant16PA6STC::JudgeCrankVibChannAlarm( float lfCharValue_,CString strFolderName_, int &AlarmCylinderNo_,float& fDistance_ )
{
...
int CDieselPlant16PA6STC::ReadNormalDataFromDisk(CString strFolderName_)
{
	//读取数据文件所在文件目录
	CString lstrFolder;
...
int CDieselPlant16PA6STC::ReadFlameOutDataFromDisk(CString strFolderName_)
{
	///读取数据文件所在文件目录
	CString lstrFolder;
...
int CDieselPlant16PA6STC::ReadCylinderKnockDataFromDisk(CString strFolderName_)
{
	///读取数据文件所在文件目录
	CString lstrFolder;
...
int CDieselPlant16PA6STC::ReadWattAbrasionDataFromDisk(CString strFolderName_)
{
	///读取数据文件所在文件目录
	CString lstrFolder;
...
int CDieselPlant16PA6STC::ReadCrankAlarmDataFromDisk(CString strFolderName_)
{
	///读取数据文件所在文件目录
	CString lstrFolder;
...
bool CDieselPlant16PA6STC::SetWavePtNum( const int nWavePtNum_ )
{
	this->m_nWaveNum = nWavePtNum_;
	return this->m_nWaveNum > 0;
}

bool CDieselPlant16PA6STC::SetTowCycleWavePtNum( const int nWavePtNum_ )
{
	this->m_nTwoCyclePtNum = nWavePtNum_;
	return this->m_nTwoCyclePtNum > 0;
}

int CDieselPlant16PA6STC::UpdateNormalWaveData( const vector<float>& avfData_ )
{
	int lnDataSize = GetInitStableVectorSpaceSize();
...
int CDieselPlant16PA6STC::UpdateNormalFreqData( const vector<float>& avfData_ )
{
	int lnDataSize = GetInitStableVectorSpaceSize();
...
int CDieselPlant16PA6STC::ReadNormalWaveDataFromDisk( CString strFolderName_ )
{
	//读取数据文件所在文件目录
	CString lstrFolder;
	...
	if(m_fNormalWaveData.size() > 0 && m_fNormalWaveData[0].size()>=20)
	{
		return 1;
	}
...
int CDieselPlant16PA6STC::ReadNormalSpectrumDataFromDisk( CString strFolderName_ )
{
	//读取数据文件所在文件目录
	CString lstrFolder;
	...
	if(m_fNormalSpectrumData.size() > 0 && m_fNormalSpectrumData[0].size()>=20)
	{
		return 1;
	}
...
void CDieselPlant16PA6STC::ClearData()
{
	m_fNormalData.clear();
	m_fFlameOutData.clear();
	m_fCylinderKnockData.clear();
...
vector<float> CDieselPlant16PA6STC::CalcDataSegEnergyPercentage( float *pData_,int nWaveNum_,int nSegNum_ )
{
...
float CDieselPlant16PA6STC::CalcDataSimilarity( vector<float> avfWave1_,vector<float> avfWave2_ )
{
	float lfResult = 0;
	...
	std::sort(avfWave1_.begin(),avfWave1_.end(),std::greater<float>());
	std::sort(avfWave2_.begin(),avfWave2_.end(),std::greater<float>());
...
float CDieselPlant16PA6STC::CalcAverageSimilarity( const vector<float>& avfData_,E_DIESEL_CALC_TYPE eType_ )
{
	float lfResult = 0;
...
int CDieselPlant16PA6STC::UpdateNormalCrankData( const float afValue_ )
{
...
int CDieselPlant16PA6STC::ReadNormalCrankDataFromDisk( CString strFolderName_ )
{
	///读取数据文件所在文件目录
	CString lstrFolder;
	...
	return m_fNormalCrankData.size() >= 20;
}

E_DIESEL_FAULT CDieselPlant16PA6STC::JudgeCrankDataAlarm( float afValue_ )
{
...
float CDieselPlant16PA6STC::CalcWaveMeanAndStdValue( const vector<float>& avfWave_,float& afMeanValue_,float& afVarValue_ )
{
	float lfMeanValue = 0;
	float lfVarValue = 0;
	if(avfWave_.size() <= 0)
	...
	afVarValue_ = lfVarValue/(avfWave_.size()-1);

	return 1;

}

int CDieselPlant16PA6STC::JudgeMeanAndVarValueAlarm( const vector<float>& avfMeanValue_,const vector<float>& avfVarValue_,const vector<float>&avfDataValue_ )
{
...
vector<float> CDieselPlant16PA6STC::CalcDataSegEnergyMeanValue( float *pData_,int nWaveNum_,int nSegNum_ )
{
	vector<float> lvfEnergyPercentage;
...
bool CDieselPlant16PA6STC::SetSampleFreq( const int nSampleFreq_ )
{
