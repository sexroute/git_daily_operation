...
GetHandlesMap(DWORD adwProcessId,std::map<HANDLE,NT::SYSTEM_HANDLE_TABLE_ENTRY_INFO_EX> & arefMap)
{
...
BOOL GetAllSocketsHandle(HANDLE ahProcessHandle,std::map<HANDLE,HANDLEDATA> & arefoHandles)
{
...

	{
		...
		BOOL lbIsSrcValid = FALSE;

		BEGIN_ERROR_HANDLE;

		lbIsSrcValid =DuplicateHandle(ahProcessHandle,
			lhHandleSystem,
			ahProcessHandle,
			&lhHandle,
			0,
...
typedef struct _LINK_HANDLE_TIME_STATE 
{
	...
	CBHDateTime m_strDate;
	_LINK_HANDLE_TIME_STATE()
	{
		m_strDate= CBHDateTime(1980,1,1,1,1,1);
	}
	...
	while(TRUE)
	{
			...
			loIt++)
		{
			...
			{				
				...
				lpHandleState->m_strDate = CBHDateTime::GetCurrentTime();

			}else if(lpData->m_oTcpLinkInfo.dwState == MIB_TCP_STATE_CLOSE_WAIT)
			{
				lpHandleState->m_strDate = CBHDateTime(1980,1,1,1,1,1);
			}

			COleDateTimeSpan loSpan = (CBHDateTime::GetCurrentTime()-lpHandleState->m_strDate);
		...
		for (TYPE_MAP_CSTRING_LINKSTATUS::iterator lpItLinks = loLinks.begin();
			lpItLinks != loLinks.end();lpItLinks++)
		{
			std::vector<LINK_HANDLE_TIME_STATE> loTempHandles(lpItLinks->second.size());
			int lnDataCount = 0;

			for (TYPE_MAP_LINKSTATUS::iterator lpLinkStatus = lpItLinks->second.begin();
				lpLinkStatus!= lpItLinks->second.end();
				lpLinkStatus++
				)
			{
				std::map<HANDLE,HANDLEDATA>::iterator lpItHandleData = loHandles.find(lpLinkStatus->first);

				TYPE_MAP_LINKSTATUS::iterator lpItPrevious = lpLinkStatus;

				BOOL lbShouldBreak = FALSE;

				if (lpItPrevious++ ==lpItLinks->second.end())
				{
					lbShouldBreak = TRUE;
					break;
				}

				if (lpItHandleData == loHandles.end())
				{
					lpItLinks->second.erase(lpLinkStatus);	

					if (lbShouldBreak)
					{
						break;
					}
					lpLinkStatus = lpItPrevious;
				}else
				{					
					loTempHandles[lnDataCount] = lpLinkStatus->second;
					lnDataCount++;
				}


			}
			BOOL lbShouldBreakOuter = FALSE;
			TYPE_MAP_CSTRING_LINKSTATUS::iterator lpItLinkStatusPrevious = lpItLinks;

			
			
			if (lpItLinkStatusPrevious++ ==loLinks.end())
			{
				lbShouldBreakOuter = TRUE;
			}

			int lnLinkSize = lpItLinks->second.size();

			if (lnLinkSize>gLinksPerPeerLimit && gLinksPerPeerLimit >0 )
			{
				sort(loTempHandles.begin(),loTempHandles.end(),TimeSort);
				int lnDataCount =0;
				for (int i =0;i<loTempHandles.size();i++)
				{
					HANDLE lhHandle = loTempHandles.at(i).m_hHandle.m_hHandle;

					MIB_TCPROW loTcpLinkInfo = loTempHandles.at(i).m_hHandle.m_oTcpLinkInfo;

					CString lstrRemote = inet_toStr(loTcpLinkInfo.dwRemoteAddr);
					CString lstrLocal =  inet_toStr(loTcpLinkInfo.dwLocalAddr);
					int lnRemotePort = ntohs(loTempHandles.at(i).m_hHandle.m_oTcpLinkInfo.dwRemotePort);
					int lnLocalPort = ntohs(loTempHandles.at(i).m_hHandle.m_oTcpLinkInfo.dwLocalPort);
					

					if (lnRemotePort == 1433)
					{
						continue;
					}				

					if (loTempHandles.at(i).m_hHandle.m_oTcpLinkInfo.dwRemoteAddr == loTempHandles.at(i).m_hHandle.m_oTcpLinkInfo.dwLocalAddr)
					{
						continue;
					}

					if ((loTempHandles.size()-i+1)<=gLinksPerPeerLimit)
					{
						continue;
					}

					if (lnLocalPort!= lnMonitorPort)
					{
						continue;
					}
					...
					lstrLog.Format(_T("%8x closing %s\r\n"),lhHandle, loTempHandles.at(i).m_strDate.Format(_T("%H:%M:%S")));

					BHLOG_DEBUG(lstrLog);
					
					if (i<gLinksPerPeerLimit)
					{
						//continue;
					}
					SOCKET loS = (SOCKET)lhHandle;
					BEGIN_ERROR_HANDLE;

					shutdown(loS,SD_BOTH);
					...
					{
						lstrLog.Format(_T("%8x close failed\r\n"),lhHandle);
			...
			else if (lpItLinks->second.size()==0)
			{
				loLinks.erase(lpItLinks);

				if (lbShouldBreakOuter)
				{
					break;
				}

				lpItLinks = lpItLinkStatusPrevious;
			}

		}

		DWORD ldwWtRet = WaitForSingleObject(gHEvent,gMilliSeconds);

		if (ldwWtRet == WAIT_OBJECT_0)
		{
			break;;
		}
