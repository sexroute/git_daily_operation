...
class AFX_EXT_CLASS CCustomReg : public CObject
{
// Construction
public:
	CCustomReg (BOOL bAdmin=TRUE, BOOL bReadOnly=FALSE);
	virtual ~CCustomReg();
...
public:
	BOOL IsOpen();
	bool SetReg( LPCTSTR pszKey , const char *pszValue);
	bool DeleteReg( LPCTSTR pszKey );
	CString GetReg( LPCTSTR pszKey );
