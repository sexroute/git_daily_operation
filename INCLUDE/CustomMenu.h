#ifndef CustomMenu_H_INCLUDE
#define CustomMenu_H_INCLUDE

#include <afxtempl.h>

// CCustomMenuData class. Fill this class structure to define a single menu item:

class CCustomMenuData
{
	wchar_t *m_szMenuText;
public:
	CCustomMenuData () {menuIconNormal=-1;xoffset=-1;bitmap=NULL;pContext=NULL;
...
class AFX_EXT_CLASS CCustomMenu : public CMenu  // Derived from CMenu
{
	struct ToolBarData	
	...
	public:
		...
		// Attributes
	protected:
		CTypedPtrArray<CPtrArray, CCustomMenuData*> m_MenuList;  // Stores list of menu items 
		
		// When loading an owner-drawn menu using a Resource, CCustomMenu must keep track of
		// the popup menu's that it creates. Warning, this list *MUST* be destroyed
		// last item first :)
		CTypedPtrArray<CPtrArray, HMENU>  m_SubMenus;  // Stores list of sub-menus 
		
		// Stores a list of all CCustomMenu's ever created 
	...
	public: 
		...
		//{{AFX_VIRTUAL(C C u s t o m M e n u)
		//}}AFX_VIRTUAL 
		// Implementation
	public:
		...
		CCustomMenuData *CCustomMenu::FindMenuItem(UINT nID);
		CCustomMenu *FindMenuOption(int nId,int& nLoc);
		CCustomMenuData *FindMenuOption(wchar_t *lpstrText);
		...
		virtual ~CCustomMenu();  // Virtual Destructor 
		// Drawing: 
		...
		CCustomMenuData *NewODMenu(UINT pos,UINT nFlags,UINT nID,CString string);
		void SynchronizeMenu(void);
		void CCustomMenu::InitializeMenuList(int value);
		void CCustomMenu::DeleteMenuList(void);
		CCustomMenuData *CCustomMenu::FindMenuList(UINT nID);
		...
		CCustomMenu* AppendODPopupMenuA(LPCSTR lpstrText);
		CCustomMenu* AppendODPopupMenuW(wchar_t *lpstrText);
