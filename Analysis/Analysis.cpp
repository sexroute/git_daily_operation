...
static AFX_EXTENSION_MODULE AnalysisDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Analysis.DLL 正在初始化！\n");
		
		// 扩展 DLL 一次性初始化
		if (!AfxInitExtensionModule(AnalysisDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(AnalysisDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Analysis.DLL 正在终止！\n");

		// 在调用析构函数之前终止该库
		AfxTermExtensionModule(AnalysisDLL);
	}
	return 1;   // 确定
}
...
void SpectralMeasureMent(float *pwave,int n,float *pout,int type) 
{
	...
	niWave.SetSize(n);
	int iAmpLen=n/2;
	niPhase.SetSize(iAmpLen);
	niAmp.SetSize(iAmpLen);
	//系数
	float fCoef=1;
	...
	CNiMath::ScaledWindow(niWave, wc, CNiMath::BlackmanHarris7Term);
	double df;
	switch(type)
	{
	case EA_POWER_SPEC:
		CNiMath::AutoPowerSpectrum(niWave,niAmp,df);
		fCoef=1;
		break;
	case EA_PEAK_SPEC:
	...
	for (nloop=2;nloop<iAmpLen;++nloop){
        pout[nloop]=fCoef*niAmp[nloop];
	}
