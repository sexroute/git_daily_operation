/*!\file CRotationRadialDisComputer.cpp
*  \brief
*  \date 2009-2-16
*  \author 
*  \version
...
// 计算特征值
void CRotationRadialDisComputer::Compute(const CNiReal32Vector &nivfWave_, const CNiReal32Vector &nivfFFT_, float fDt_, float fFref_, const CNiReal32Vector &nivfParam_, CNiReal32Vector &nivfEV_)
{
	/* 业务流程：
		1. 求通频峰峰值
		2. 求各倍频幅值
	...
	*/
	...
	// 1
	nivfEV_[IDX_EV_ALL] = CKDFilter::GetPeak2Peak(nivfWave_);	
	if (fFref_ <= 3.1*df || nivfEV_[IDX_EV_ALL] < 0.1 || fFref_*6 > iSampleRate)// 如果信号为0或者信号频率小于3.1df时，频谱校正算法异常，所以增加此判断
	{
		...
		nivfEV_[IDX_EV_TWO] = CKDFilter::GetFreqAmplitude(nivfFFTtemp, df, fFref_, 2, 3, f[2]);	// 2倍频幅值
		nivfEV_[IDX_EV_THREE] = CKDFilter::GetFreqAmplitude(nivfFFTtemp, df, fFref_, 3, 3, f[3]);	// 3倍频幅值
	...
	else if (fFref_ > 3.1*df && fFref_ <= 6.1*df && nivfEV_[IDX_EV_ALL] >= 0.1 && fFref_*6 <= iSampleRate)
	{
		...
		SigMath.GetCalibratedSpectrumCharInfo(nivftemp, fSpecCorrectFreq_, iSampleRate, nivftemp.GetSize(), vSigComponet, E_SpectrumType_Peak_Peak);		
	...
	// 4
	nivfEV_[IDX_EV_RESVIB] = CKDFilter::GetResVib(nivfFFT_, df, fFref_, 10);	// 残振
	// 5
	...
	if (!CKDFilter::IsFloatValid(nivfEV_[IDX_EV_ONE]))
	{
		...
		CHZLogManage::Error(str, _T("CRotationRadialDisComputer"), _T("[工频幅值(有效值)]异常"));
	...
	if (!CKDFilter::IsFloatValid(nivfEV_[IDX_EV_ONE_PHASE]))
	{
		...
		CHZLogManage::Error(str, _T("CRotationRadialDisComputer"), _T("[工频相位]异常"));
	...
	if (!CKDFilter::IsFloatValid(nivfEV_[IDX_EV_TWO]))
	{
		...
		CHZLogManage::Error(str, _T("CRotationRadialDisComputer"), _T("[2倍频幅值]异常"));
	...
	if (!CKDFilter::IsFloatValid(nivfEV_[IDX_EV_TWO_PHASE]))
	{
		...
		CHZLogManage::Error(str, _T("CRotationRadialDisComputer"), _T("[2倍频相位]异常"));
	...
	if (!CKDFilter::IsFloatValid(nivfEV_[IDX_EV_THREE]))
	{
		...
		CHZLogManage::Error(str, _T("CRotationRadialDisComputer"), _T("[3倍频幅值]异常"));
	...
	if (!CKDFilter::IsFloatValid(nivfEV_[IDX_EV_THREE_PHASE]))
	{
		...
		CHZLogManage::Error(str, _T("CRotationRadialDisComputer"), _T("[3倍频相位]异常"));
	...
	if (!CKDFilter::IsFloatValid(nivfEV_[IDX_EV_HALF]))
	{
		...
		CHZLogManage::Error(str, _T("CRotationRadialDisComputer"), _T("[0.5倍频幅值]异常"));
	...
	if (!CKDFilter::IsFloatValid(nivfEV_[IDX_EV_HALF_PHASE]))
	{
		...
		CHZLogManage::Error(str, _T("CRotationRadialDisComputer"), _T("[0.5倍频相位]异常"));
