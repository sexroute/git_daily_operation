...
#ifdef _DEBUG
  #pragma  comment(lib,"tracetoold.lib")
#else
...
public:
   ...
   void UnIndent    (const char    *leftMsg = NULL, const char    *rightMsg = NULL) ;
   void UnIndent    (const wchar_t *leftMsg = NULL, const wchar_t *rightMsg = NULL) ;
...
public :
   ...
   void SaveToXml       (const char    * FileName,const char    * styleSheet) ;  // Save the window tree traces to an XML file and specifying a styleSheet
   void SaveToXml       (const wchar_t * FileName,const wchar_t * styleSheet) ;  // Save the window tree traces to an XML file and specifying a styleSheet
...
private:
   ...
   static void ParseForInternal(CommandList * Commands) ;
...
public:
   ...
   static string HtmlEncode (string source) ;   // helper function

   ///单独调用
   static void InitTrace(const char * WinTraceID , const char * WinTraceTitle);

   /// Shortcut to WinTrace.Debug
   /// <returns>WinTrace.Debug</returns>
   static TraceToSend    * Debug();
   static void DebugEx(const char *cFunctionName_,const char *cFileName_, const int iLineNo_, int len, const char *cmd, ...);
   static void ErrorEx(const char *cFunctionName_,const char *cFileName_, const int iLineNo_, int len, const char *cmd, ...);
   static void DebugEx(const int& len, const char *cmd, ...);
   static void ErrorEx(const int& len, const char *cmd, ...);
...
///单独定义marble
#ifdef _DEBUGEX
  #define  BHTRACE_Init(infoid,info) {TTrace::InitTrace(infoid,info);}
  #define  BHTRACE_Enter_Method  { TTrace::Debug()->EnterMethod(__FUNCTION__);}
  #define  BHTRACE_Exit_Method  { TTrace::Debug()->ExitMethod(__FUNCTION__);}
  #define  BHTRACE_DEBUG(info) \
           { \
		   char cInfo_trace[1024]={0};\
		   sprintf(cInfo_trace, "%s  \nFileName:%s  \n line: %d",__FUNCTION__, __FILE__, __LINE__); \
           TTrace::Debug()->Send(info,cInfo_trace);\
		   }
#else
  #define  BHTRACE_Init(infoid,info) {TTrace::InitTrace(infoid,info);}
  #define  BHTRACE_Enter_Method 
  #define  BHTRACE_Exit_Method
  #define  BHTRACE_DEBUG(info)
#endif
